    ...
    <div class="articlesList">
        <? foreach ($arResult['ITEM_ROWS'] as $rowData) {
            ...
            <? foreach ($rowItems as $item) { ?>
                <a class="articlesList__item clearfix" href="<?=$item['DETAIL_PAGE_URL']?>">

                    <?$fileImg = CFile::ResizeImageGet($item["PREVIEW_PICTURE"]["ID"], array('width'=>270, 'height'=>200), BX_RESIZE_IMAGE_EXACT, false);
                    $item["PREVIEW_PICTURE"]["SRC"] = $fileImg["src"]; ?>
                    <img class="articlesList__photo" src="<?=$fileImg["src"]?>">
                    <div class="articlesList__title"><?=$item['NAME']?></div>
                    <div class="articlesList__description"> <?=TruncateText($item["PREVIEW_TEXT"], 200);?></div>
                </a>
            <?}
        }
